...
class AddArticleForm extends FormBase
{
    ...
    private function title()
    {
        ...
        $element->addValidator(new StringLength([
            "max" => 80,
            "maxMessage" => "Le titre de l'article ne peut dépasser les 80 caractères."
        ]));
        return $element;
    }
    ...
    private function slug()
    {
        ...
        $element->addValidator(new StringLength([
            "max" => 80,
            "maxMessage" => "L'url de l'article ne peut dépasser les 80 caractères."
        ]));
        $element->addValidator(new Regex([
            "pattern" => '/^[a-z0-9\-]+$/i',
            "message" => "L'url de l'article ne peut contenir que des lettres et chiffres (de A à Z et de 0 à 9) et des tirets (-).",
            "allowEmpty" => true,
        ]));
        return $element;
    }
    ...
    private function datePublication()
    {
        $element = new Text("datePublication");
        $element->setLabel("Date de publication");
        $element->addValidator(new Date([
            "format" => "d/m/Y",
            "message" => "La date est invalide (format attendu : jj/mm/aaaa)"
        ]));
        return $element;
    }
